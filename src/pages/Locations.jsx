...
function QRModal({ location, onClose }) {
  const reviewUrl = location.place_id
    ? `https://search.google.com/local/writereview?placeid=${location.place_id}`
    : `https://www.google.com/search?q=${encodeURIComponent((location.name ?? '') + ' ' + (location.address ?? ''))}&action=reviews`;

  const qrSrc = `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(reviewUrl)}`;

  function handleDownload() {
    const link = document.createElement('a');
    link.href = qrSrc;
    link.download = `${(location.name ?? 'location').replace(/\s+/g, '-')}-review-qr.png`;
    link.click();
  }
    ...
    >
      <div style={{
        ...
        <div style={{
          ...
          <img src={qrSrc} width={200} height={200} alt="QR code" style={{ display: 'block' }}/>
        </div>

        <div style={{
          background: 'var(--bg-muted)', borderRadius: 'var(--radius-md)',
          padding: '12px 16px', marginBottom: 20,
          fontSize: 12, color: 'var(--ink-2)', lineHeight: 1.6, textAlign: 'left',
        }}>
          Print this QR code and place it on your tables, receipts, or at the register.
          Customers scan it to leave a Google review instantly.
        </div>
...
export default function Locations() {
  ...
  const usagePct     = maxLocations > 0 && maxLocations < 999
    ? Math.round((activeCount / maxLocations) * 100) : 0;
          ...
          {locations.map((loc, i) => {
            ...
            return (
              <div key={loc.id} style={{
                ...
                {/* Top row */}
                <div style={{ display:'flex', alignItems:'flex-start',
                  gap:12, marginBottom: isActive ? 14 : 0 }}>
                  <div style={{
                ...
                {/* Stats + QR row — only for active locations */}
                {isActive && (
                  <div style={{
