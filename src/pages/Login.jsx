...
const TIERS = [
  { tier: 'Starter',    locations: '1-3 locations',   monthly: 29, yearly: 290, color: '#1D9E75', popular: false },
  { tier: 'Growth',     locations: '4-9 locations',   monthly: 19, yearly: 190, color: '#185FA5', popular: true  },
  { tier: 'Agency',     locations: '10-24 locations', monthly: 15, yearly: 150, color: '#534AB7', popular: false },
  { tier: 'Enterprise', locations: '25+ locations',   monthly: 12, yearly: 120, color: '#993C1D', popular: false },
];
    ...
    <div style={{ background: 'var(--bg)', minHeight: '100vh', fontFamily: 'var(--font-sans)' }}>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '80px 48px 60px', textAlign: 'center' }}>
        ...
        <h1 style={{
          ...
          color: 'var(--ink)', lineHeight: 1.1, marginBottom: 20, letterSpacing: '-0.02em',
        }}>
          Your restaurant reviews,<br />
          <span style={{ color: '#1D9E75' }}>actually responded to.</span>
        </h1>

        <p style={{ fontSize: 18, color: 'var(--ink-2)', lineHeight: 1.7, maxWidth: 560, margin: '0 auto 40px' }}>
      ...
      {/* Stats strip */}
      <div style={{ background: 'var(--ink)', padding: '32px 48px', display: 'flex', justifyContent: 'center', gap: 80, flexWrap: 'wrap' }}>
        {[
          ...
          return (
            <div key={s.stat} style={{ textAlign: 'center' }}>
              <div style={{ fontFamily: 'var(--font-display)', fontSize: 36, color: '#F7F5EF', fontWeight: 400, lineHeight: 1 }}>{s.stat}</div>
              <div style={{ fontSize: 13, color: 'rgba(247,245,240,0.45)', marginTop: 4 }}>{s.label}</div>
            </div>
          );
        })}
      </div>
      ...
      {/* Features */}
      <div id="features" style={{ maxWidth: 900, margin: '0 auto', padding: '80px 48px' }}>
        <div style={{ textAlign: 'center', marginBottom: 48 }}>
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 36, fontWeight: 400, color: 'var(--ink)', marginBottom: 12 }}>
            Everything you need to manage your reputation
          </h2>
          <p style={{ fontSize: 16, color: 'var(--ink-2)', maxWidth: 500, margin: '0 auto' }}>
          ...
          {FEATURES.map(function(f) {
            return (
              <div key={f.title} style={{
                ...
                <p style={{ fontSize: 13, color: 'var(--ink-2)', lineHeight: 1.6, margin: 0 }}>{f.body}</p>
              </div>
            );
          })}
        </div>
      ...
      {/* How it works */}
      <div style={{ background: 'var(--bg-card)', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)', padding: '80px 48px' }}>
              ...
              return (
                <div key={s.step} style={{ textAlign: 'center' }}>
                  <div style={{
                    ...
                    margin: '0 auto 16px', fontFamily: 'var(--font-display)', fontSize: 20, color: '#1D9E75',
                  }}>
                    {s.step}
                  </div>
                  <div style={{ fontSize: 15, fontWeight: 500, color: 'var(--ink)', marginBottom: 8 }}>{s.title}</div>
                  <p style={{ fontSize: 13, color: 'var(--ink-2)', lineHeight: 1.6, margin: 0 }}>{s.body}</p>
                </div>
              );
            })}
          </div>
      ...
      {/* Pricing */}
      <div id="pricing" style={{ maxWidth: 900, margin: '0 auto', padding: '80px 48px' }}>
        <div style={{ textAlign: 'center', marginBottom: 40 }}>
          ...
          <div style={{ display: 'inline-flex', alignItems: 'center', background: 'var(--bg-muted)', borderRadius: 99, padding: 4, border: '1px solid var(--border)' }}>
            {['Monthly', 'Annual'].map(function(t) {
              var isAnnual = t === 'Annual';
              return (
                <button key={t} onClick={function() { setAnnual(isAnnual); }} style={{
                  padding: '6px 20px', borderRadius: 99, fontSize: 13, fontWeight: 500,
                  cursor: 'pointer', border: 'none',
                  background: isAnnual === annual ? 'var(--ink)' : 'transparent',
                  color: isAnnual === annual ? 'var(--bg)' : 'var(--ink-2)',
                  transition: 'all 0.15s',
                }}>
                  {t}
                  {isAnnual && (
                    <span style={{ marginLeft: 6, fontSize: 10, color: annual ? '#1D9E75' : 'var(--ink-3)', fontWeight: 600 }}>
                      SAVE 17%
                    </span>
                  )}
                </button>
          ...
          {TIERS.map(function(tier) {
            return (
              <div key={tier.tier} style={{
                ...
                border: tier.popular ? '2px solid #185FA5' : '1px solid var(--border)',
                borderRadius: 'var(--radius-lg)', padding: '24px 20px',
                position: 'relative', boxShadow: 'var(--shadow-sm)', textAlign: 'center',
              }}>
                {tier.popular && (
                  <div style={{
                    position: 'absolute', top: -12, left: '50%', transform: 'translateX(-50%)',
                    background: '#185FA5', color: '#fff', fontSize: 10, fontWeight: 600,
                    padding: '3px 12px', borderRadius: 99, letterSpacing: '0.05em', whiteSpace: 'nowrap',
                  }}>
                    MOST POPULAR
                  </div>
                )}
                <div style={{ fontSize: 15, fontWeight: 500, color: 'var(--ink)', marginBottom: 4 }}>{tier.tier}</div>
                <div style={{ fontSize: 12, color: 'var(--ink-3)', marginBottom: 20 }}>{tier.locations}</div>
                <div style={{ fontFamily: 'var(--font-display)', fontSize: 36, color: tier.color, fontWeight: 400, lineHeight: 1, marginBottom: 4 }}>
                  ${annual ? tier.yearly : tier.monthly}
                </div>
                <div style={{ fontSize: 11, color: 'var(--ink-3)', marginBottom: 24 }}>
                  per location / {annual ? 'year' : 'month'}
                </div>
                <a href={BASE + '/auth/google?plan=' + tier.tier.toLowerCase()} style={{
                  display: 'block', padding: '10px', borderRadius: 'var(--radius-md)',
                  background: tier.popular ? '#185FA5' : 'var(--ink)',
      ...
      <div style={{ background: 'var(--ink)', padding: '80px 48px', textAlign: 'center' }}>
        <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 36, fontWeight: 400, color: '#F7F5EF', marginBottom: 16 }}>
          Ready to respond to every review?
        </h2>
        <p style={{ fontSize: 16, color: 'rgba(247,245,240,0.55)', maxWidth: 480, margin: '0 auto 32px' }}>
          Join restaurants across Utah using reviewzhealth to build their reputation, one reply at a time.
        </p>
        <a href={BASE + '/auth/google'} style={{
          display: 'inline-flex', alignItems: 'center', gap: 10,
          padding: '14px 32px', borderRadius: 'var(--radius-md)',
          background: '#1D9E75', color: '#fff', fontSize: 16, fontWeight: 500, textDecoration: 'none',
        }}>
          <GoogleIcon /> Get started free
        </a>
      </div>
